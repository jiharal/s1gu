...
func CreateNewCMD(appPath, appName string) string {
	...
	import (
		...
		"` + fmt.Sprintf("%s/auth", appPath) + `"
		"` + fmt.Sprintf("%s/router", appPath) + `"
	...
	func init() {
		...
		rootCmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "", "config file(default is $HOME/.` + fmt.Sprintf("%s", appName) + `.config.toml)")
